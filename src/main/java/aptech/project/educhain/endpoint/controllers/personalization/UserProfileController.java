...
public class UserProfileController {
    ...
    public ResponseEntity<?> getUser(HttpServletRequest request) {
        ...
        ApiError errorRes;
        if (token == null) {
            errorRes = new ApiError("error when take token form header");
            return ResponseEntity.badRequest().body(errorRes);
        }
        String newToken = token.substring(7);
        var email = iJwtService.extractUserName(newToken);
        if (email == null) {
            errorRes = new ApiError("invalid token from header");
            return ResponseEntity.badRequest().body(errorRes);
        }
    ...
    public ResponseEntity<?> getAwardByUserIdAndAwardId(@RequestBody UserHomeworkRequest req) {
        ...
        if (result.isSuccess()) {
            var res = result.getSuccess()
                    .map(userHomeworkDTO -> modelMapper.map(userHomeworkDTO, UserHomeworkResponse.class));
            return ResponseEntity.ok().body(res);
        }
        return ResponseEntity.badRequest().body(result.getFailure().getMessage());
    }
